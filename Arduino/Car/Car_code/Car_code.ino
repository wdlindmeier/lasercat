...
void ControlLeft(int val, int dt) {
...
void ControlLeftReverse(int val, int dt) {
  Serial.println("+Control left reverse");
  // speed
  analogWrite(SpeedPin1,val);
  analogWrite(SpeedPin2,val);
...
void ControlRightReverse(int val, int dt) {
  Serial.println("+Control right reverse");
  // speed
  analogWrite(SpeedPin1,val);
  analogWrite(SpeedPin2,val);
  digitalWrite(MotorRight2,HIGH); //reverse
  //delay(dt* 100);
}


void ControlRight(int val, int dt) {
  Serial.println("+Control right");
  // speed
  analogWrite(SpeedPin1,val);
  analogWrite(SpeedPin2,val);
  digitalWrite(MotorRight1,HIGH); //reverse
  //delay(dt* 100);
}
...
{   
  ...
  /* 
  // Data format to receive from IR 
  // long val = 255,255;
  // most significant three bits are left wheel speed+direction from 1-500
  // least significant three bits are right wheel speed+direction from 1-500
  // for every value, range from 1-250 indicates forward and 251-500 indicates reverse
  */
#ifdef USE_CN70
  ...
  { 
    ...
    //if (val < 500500 && val > 0){
        ...
        ControlLeft(lVal, delayTime);
        
/*
        if (lVal > 0 && lVal < 250) {
        // reverse
            ControlLeftReverse(lVal, delayTime);
        }
        else if (lVal > 250 && lVal < 500) {
           lVal = lVal % 255; 
           ControlLeft(lVal, delayTime);
        } 
       
        else if (rVal > 0 && rVal < 250) {
        // reverse
            ControlRightReverse(rVal, delayTime);
        }
        else if (rVal > 250 && rVal < 500) {
            rVal = rVal % 255;
            ControlRight(rVal, delayTime);
        }
